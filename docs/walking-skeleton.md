...
| A2UI compilation | Server compiles validated draft → A2UI v0.9.1 messages from the allowlisted catalog | No |
...
| Student UI | Responsive matching screen, multi-select, EN/AR toggle, RTL, keyboard path, reduced motion | No |
...
5. **WCAG 2.2 AA:** keyboard-complete path; no canvas/ink-only interaction; reduced-motion respected.
